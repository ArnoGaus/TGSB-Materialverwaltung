  ...
  return (
    <div className="modal-overlay">
      <div className="modal-box">
        ...
        {/* ✅ Nur Admins dürfen den Namen ändern (Input + Button nur dann rendern) */}
        {isAdmin && (
          <div
            style={{
              marginTop: 12,
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {f.type === "select" && (
                        <select
                          ...
                          value={current ?? ""}
                          onChange={(e) =>
                            setField(f.key, e.target.value, "select")
                          }
                        >
                          {(f.options || [""]).map((opt) => (
        ...
        >
          {!edit ? (
            <>
              {/* ✅ "Ändern" ohne inline-style => nimmt Standard button aus app.css */}
              {canEdit && <button onClick={() => setEdit(true)}>Ändern</button>}

              {/* ✅ Secondary über class, nicht inline */}
